...
def main():
    ...
    for i, code in enumerate(codes):
        try:
            ...
            if signal:
                ...
                k_change = signal["kChange"]
                dif_change = signal["difChange"]
                print(f"[{i+1}/{len(codes)}] ✓ {signal['tsCode']} | 最低:{signal['minLow']} @ {signal['minLowTime'][8:]} | DIF: {signal['difBefore']:.4f} -> {signal['difNow']:.4f} (+{dif_change:.4f}) | K: {signal['kBefore']:.2f} -> {signal['kNow']:.2f} (+{k_change:.2f})")
    ...
    print(f"\n{'='*80}")
    print(f"今日 ({today}) 共找到 {len(signals)} 个信号")
    print(f"{'='*80}")
    ...
    # 按K变化排序
    signals.sort(key=lambda x: x["kChange"], reverse=True)

    print(f"\n{'代码':<12} {'最低价':>10} {'时间':>8} {'当前价':>10} {'DIF变化':>12} {'K变化':>10}")
    print(f"{'-'*80}")
    for s in signals:
        print(f"{s['tsCode']:<12} {s['minLow']:>10} {s['minLowTime'][8:]:>8} {s['currentPrice']:>10} {s['difChange']:>+12.4f} {s['kChange']:>+10.2f}")

    # 按股票汇总（去重）
    stock_signals = {}
    for s in signals:
        code = s["tsCode"]
        if code not in stock_signals or s["kChange"] > stock_signals[code]["kChange"]:
            stock_signals[code] = s

    print(f"\n去重后共 {len(stock_signals)} 只股票有信号")
    print(f"\n{'='*80}")
    print(f"{'代码':<12} {'最低价':>10} {'时间':>8} {'当前价':>10} {'DIF变化':>12} {'K变化':>10}")
    print(f"{'-'*80}")
    for s in sorted(stock_signals.values(), key=lambda x: x["kChange"], reverse=True):
        print(f"{s['tsCode']:<12} {s['minLow']:>10} {s['minLowTime'][8:]:>8} {s['currentPrice']:>10} {s['difChange']:>+12.4f} {s['kChange']:>+10.2f}")
